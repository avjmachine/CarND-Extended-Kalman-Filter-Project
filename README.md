...
A Kalman filter is basically an algorithm to estimate the state of a system such as position, orientation, velocity, pressure, temperature, voltage, etc. using a combination of physical laws and measurements. We need the filter because measurements from sensors are usually noisy and predictions based on physical laws could be erroneous due to uncertainities in initial state and processes. The Kalman filter combines inputs from both these sources using laws of probabilities, weighted using the degree of uncertainities in these sources. 

It is basically a two step process - a predict step followed by an update (or measurement) step, which is repeated cyclically. In the first predict step, the new state (for example, position) is predicted using a known physical law (based on certain assumptions such as constant velocity). The uncertainity in this state is noted down. As a second step, this state is updated based on the actual measurement (say, coming in from a position sensor). During this update step, the state (in this example, the position) is updated based on the ratio of the uncertainities in both steps. It is similar to taking a weighted average of two quantities, with the higher weight being assigned to the value with lower uncertainity and a lower weight being assigned to the value with higher uncertainity. So, the final estimated state will be closer to the predicted state if the predicted state has lower uncertainity than the measurement and vice-versa.  

This ratio which is used for the weighting is called the Kalman gain.
...
H is the measurement matrix transforming the state vector to the space of the measurement values;

R is the uncertainity in the sensor measurement;

S is the sum of the uncertainities - state covariance and the sensor noise;

K is the ratio of the uncertainity in prediction (state covariance) with respect to the sum of the uncertainities (prediction + measurement);
...
In this particular project, the predict step is not affected and will continue to be the same for both the laser and radar, the measure step for the laser will also continue to be the same, but the measure step for the radar will use the extended Kalman Filter equations due to non-linear relationship between the measurement z obtained from the radar and the state vector x. The measurement z obtained from the radar is in Polar coordinates, whereas the state vector is in Cartesian coordinates. Inorder to calculate error between prediction and measurement, the state vector needs to be transformed into polar coordinates, and this transformation is non-linear in nature and hence an Extended Kalman Filter update is required in this case.

## Brief explanation of key blocks in the code

The code consists of the following files:

1. `main.cpp` - This file contains the code to interface with the simulator - to receive the measurement data and ground truths, to supply the algorithm with the measurement values one by one, line by line (to the `FusionEKF.cpp`), to call the Kalman filter predict and update functions (in `kalman_filter.cpp`) and receive the Kalman estimated values, to call the Jacobian and the RMSE performance calculation functions (`CalculateJacobian` and `CalculateRMSE` in `tools.cpp`), and finally to send the estimates and RMSE back to the simulator using the uWebSocketIO connection.
...
3. `tools.h` and `tools.cpp` - These files contain the class headers and function definitions for the calculation of the Jacobian of the measurement matrix Hj and the root mean square measure of the error between the Kalman estimate and the ground truth.
4. `kalman_filter.h` and `kalman_filter.cpp` - These files have the headers and definition for the prediction and update steps. The Kalman filter equations can be found in this segment of the code. There is a common predict step for both the laser and the radar sensors, but a separate measurement update step for each of the laser and the radar sensors. The Update function for the laser sensor uses the normal Kalman update equations, whereas the UpdateEKF function for the radar uses the extended Kalman filter equations, where the H matrix is replaced by the Jacobian Hj matrix for all equations. The other difference between the two update functions is that in the Extended Kalman Filter, the y calculation in the radar uses a non-linear h function transforming the Cartesian state vector variables to the Polar coordinate variables instead of the usual linear transformation using H matrix.
...
The items 1, 5, 6 and 7 are not modified and are used directly, while the code in items 2,3 and 4 has been newly created or significantly modified inorder to program the Kalman filter and measure the performance. 
...
After the first measurement to initialize the vectors and matrices, for every subsequent measurement, the algorithm used in the code first predicts the object position to the current timestep and then updates the prediction using the new measurement.
